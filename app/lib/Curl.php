...
class Curl
{
     ...
     */
    public function __construct($cookie_jar_file = null)
    {
        ...
        $this->cookie_jar_file = $cookie_jar_file;
    ...
    private function init()
    {
        ...
        if($this->cookie_jar_file != false)
        {
            $this->setOpt( CURLOPT_COOKIEJAR, $this->cookie_jar_file);
            $this->setOpt( CURLOPT_COOKIEFILE, $this->cookie_jar_file);
        }
        return $this;
    }
    ...
    protected function exec()
    {
        ...
        $header_size = $this->getOpt(CURLINFO_HEADER_SIZE);
        $header = substr($this->response, 0, $header_size);
        $hedSplit = explode("\r\n", $header);
        foreach ($hedSplit as $hed)
            $this->addResponseHeaderLine($this->curl, $hed);
     ...
     * $curl = new Curl();
     * $curl->get('http://example.com/request.php');
     * $curl->getResponseHeader('location');
     * ```
     *
     * @param string $key   The header key
     * @param string $value The value for the given header key
    ...
    public function getResponseHeader($key)
    {
        $key = strtolower($key);
        $keyLen = strlen($key);
        foreach ($this->response_headers as $hed)
        {
            if(substr($hed,0, $keyLen) === $key)
            {
                list($hk, $headerValue) = explode(":", $hed, 2);
                return $headerValue;
            }
        }
        return null;
